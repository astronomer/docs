...
1. In the Airflow UI, go to **Admin**` > `**Connections** and then select the `airflow_db` connection.
...
2. Edit the following connection settings:
...
- Add the following connection information:
    ...
    port (optional): 5432
    ```
Your connection should appear similar to the following
...
3. Add the following to your DAG to connect it to `airflow_db`:
...
In this example, the task pulls all `dag_ids` from the `dag` table in the Airflow metadata database.
...
1. Open [`psql`](https://www.postgresql.org/docs/9.3/app-psql.html), a terminal-based front-end to PostgreSQL and then run:

```
psql -h localhost -U postgres -p 5432 -W
```

2. When prompted, enter the password for the connection object.

You can also use the Postgres container to connect to the database by exec-ing into it and then running the `psql` command. For example:
