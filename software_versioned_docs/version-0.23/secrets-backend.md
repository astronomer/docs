...
By default, this setup prefixes secret names in Key Vault with `airflow-connections` or `airflow-variables`. You can customize the names of these prefixes, but `"connections_prefix"` and `"variables_prefix"` must have defined values in your configuration.
