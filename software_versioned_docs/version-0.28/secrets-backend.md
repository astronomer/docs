...
:::tip
By default, this setup requires that you prefix any secret names in Key Vault with `airflow-connections` or `airflow-variables`. If you don't want to use prefixes in your Key Vault secret names, set the values for `sep` and `"connections_prefix"` and `"connections_prefix"` to `""` within `AIRFLOW__SECRETS__BACKEND_KWARGS`.
