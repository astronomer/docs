...
Initializes a new Airflow project in your working directory. The set of files generated by this command are required to run Airflow locally and can be pushed to a Deployment on Astronomer.

:::info

If you want to deploy your project to a Software Deployment, you must either specify the `--use-astronomer-certified` flag when running `astro dev init` or update the image in your project's `Dockerfile` to a supported Astronomer Certified image.
...
| `--use-astronomer-certified` | ``     | Create the new project with the latest version of Astronomer Certified. By default, your project is created with the latest version of Astronomer Runtime.  |
