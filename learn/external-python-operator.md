...
- [astro-provider-venv](https://github.com/astronomer/astro-provider-venv) makes it easy to execute Airflow tasks in virtual environments. Managing the build process when using the ExternalPythonOperator to get clean, quick Docker builds can take a lot of plumbing. This repo provides a nice packaged solution to it, that plays nicely with Docker image caching.
