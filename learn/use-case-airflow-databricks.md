...
Next, the `select_countries` task uses the Astro Python SDK [@aql.transform](https://astro-sdk-python.readthedocs.io/en/stable/astro/sql/operators/transform.html) decorator to execute a SQL query on each of the three temporary tables created in the `in_tables` task. The query selects rows only for the specified `COUNTRY` defined in `.env` and saves the results in three new temporary tables. 
