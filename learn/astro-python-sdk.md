...
3. If you're using the Astro CLI locally, add the following variable to the `.env` file of your Astro project:
    ...
    If you would like to store all the intermediary tables generated by `astro-sdk` in a schema of your choice, you can specify the schema by using variable `AIRFLOW__ASTRO_SDK__SQL_SCHEMA`. To deploy a pipeline written with the Astro Python SDK to Astro, add these environment variables to your Deployment. See [Environment variables](https://docs.astronomer.io/astro/environment-variables).
