...
First you will add a static extra link to the SimpleHttpOperator.
...
```
This DAG has one SimpleHttpOperator task that posts a GET request to API provided in the `random_user_api_conn` connection.
...
1. In your terminal, run `astro dev start` to start up Airflow. If your Airflow instance is already running use `astro dev restart` to restart it in order to load any changes made in the `/plugins` folder.
...
## Step 5: Create a custom operator
...
## Step 6: Create a DAG with your custom operator
...
## Step 7: Run your DAG and view modified XComs
...
## Step 8: Add a dynamic extra link to your custom operator
