...
This DAG consists of Six Tasks

- `import_data_func` Task: This Python task reads movie data from a text file, processes it to extract titles, years, genres, and descriptions, and then vectorizes the descriptions using TF-IDF (Term Frequency-Inverse Document Frequency) from Scikit learn.
- `vectorize` Task: This task uses OpenAI's Embedding model to generate vector embeddings for each movie description. It updates the movie data dictionaries with these vectors.
- `get_vector_dimensions` Task: This task calculates the dimension of the vector embeddings generated by the vectorize task. It returns the length of the vector for the first movie, assuming all vectors have the same dimension.
- `create_index_if_not_exists` Task: This task checks if a Pinecone index with the given name exists. If it doesn't, it creates a new index in Pinecone with the specified vector size.
- `ingestion` (PineconeIngestOperator): This operator ingests the vectorized movie data into the Pinecone index. It takes the vector data output from the vectorize task.
- `query_pinecone` Task: This task performs a vector search in Pinecone based on a user-provided mood or query (retrieved from Airflow's context). It first vectorizes the query using OpenAI's Embedding model and then queries the Pinecone index to find the most relevant movie.
