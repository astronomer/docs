...
You can trigger a downstream DAG with the TriggerDagRunOperator from any point in the upstream DAG. If you set the operator's  `wait_for_completion` parameter to `True`, the upstream DAG will pause and resume only once the downstream DAG has finished running. This waiting process can be deferred to the triggerer by setting the parameter `deferrable` to True, turning the operator into a [deferrable operator](deferrable-operators.md) which increases Airflow's scalability and can reduce cost.
