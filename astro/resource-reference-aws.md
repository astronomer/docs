...
To create a new cluster on Astro with a specified configuration, see [Install on AWS](install-aws.md) or [Create a cluster](create-cluster.md). To make changes to an existing cluster, see [Modify a cluster](modify-cluster.md). To view your clusters' current configurations, see [Manage clusters](view-clusters.md).
