...
There are some restricted versions of the Astro runtime that have known bugs, and cannot be used, even if you have [Running a deprecated Astro Runtime version](https://www.astronomer.io/docs/astro/upgrade-runtime#run-a-deprecated-astro-runtime-version) enabled. These invalid versions of the Astro Runtime have a status `"yanked":true` if you query `https://updates.astronomer.io/astronomer-runtime` and include a description about why the version is unavailable.
