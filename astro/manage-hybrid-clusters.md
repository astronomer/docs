...
In this scenario, the maximum node count for the `m5.2xlarge` node pool in your cluster must be equal to or greater than 15 to make sure that each worker queue can scale to its limit. However, setting your node pool to 20 for this example would be appropriate if you expect the cluster frequently reach maximum load.
