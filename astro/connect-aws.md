...
Use AWS PrivateLink to create private connections from Astro to your AWS services without exposing your data to the public internet.  PrivateLink endpoints can only be used to expose services in that were created in the same region as the Astro cluster.
...
To access a service in a different region using PrivateLink endpoints, an inter-region VPC Peering Connection can be used to an intermediate VPC. The intermediate VPC can be created in the same region as the targetted service, and the PrivateLink endpoint can then be created in this VPC.
