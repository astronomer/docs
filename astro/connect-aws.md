...
After you allowlist a cluster's IP addresses, all Deployments in that cluster are allowed to access your AWS resources. When you use publicly accessible endpoints to connect to AWS, traffic moves directly between your Astro cluster and the AWS API endpoint. Data in this traffic never reaches the Astronomer managed control plane.
