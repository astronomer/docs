...
Use AWS PrivateLink to create private connections from Astro to your AWS services without exposing your data to the public internet.  PrivateLink endpoints can only be used to expose services that were created in the same region as the Astro cluster.
...
To access a service in a different region using PrivateLink endpoints, you can use an inter-Region VPC peering connection. You create the inter-Region VPC peering connection in the same region as the targeted service, and then you create the PrivateLink endpoint in the VPC.
