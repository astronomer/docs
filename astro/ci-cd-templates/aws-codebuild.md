...
If you use the [DAG-only deploy feature](deploy-code#deploy-dags-only) on Astro and are interested in a DAG deploy CI/CD template, see [Template overview](template-overview.md) to configure your own. To learn more about CI/CD on Astro, see [Choose a CI/CD strategy](set-up-ci-cd.md).
