...
By default, all tasks using the Celery executor run in a `default` worker queue. You can create additional worker queues to enable multiple worker types or configurations for different groups of tasks, and assign tasks to these queues in your DAG code. For more information about [Apache Airflow®](https://airflow.apache.org) executors on Astro, see [Manage Airflow executors on Astro](executors-overview.md).
...
<details>
  <summary><strong>Astro Hybrid worker type setup</strong></summary>
...
For more information on limited parallelism, see [Airflow Pools](https://www.astronomer.io/docs/learn/airflow-pools).
