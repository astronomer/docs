...
The Airflow UI now shows the Deployment's Docker image tag in the footer of all pages. Depending on how your team deploys to Astro, this value is either a unique identifier generated by a CI tool or a timestamp generated by the Astro CLI on `$ astrocloud deploy`. Both represent a unique version of your Astro project.

![Runtime Tag banner](image-tag.png)
...
The Docker image tag shown in the Airflow UI is based on the image running in your Deployment's Webserver. After a code deploy, this image might briefly be different from the image running on your Scheduler, Workers, or Triggerer. If you ever have trouble verifying a code push to a Deployment on Astro, reach out to [Astronomer support](https://support.astronomer.io).
