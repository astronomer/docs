...
- `astro organization user update`: Update a user's Organization-level role.
...
| `--role`  | The user's role in the Organization. Use only with `astro organization user update` and `astro organization user invite`.                         | Possible values are either `ORGANIZATION_MEMBER`, `ORGANIZATION_BILLING_ADMIN`, or `ORGANIZATION_OWNER`.   Default is `ORGANIZATION_MEMBER` |
