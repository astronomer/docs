...
You can use template and Deployment files to define Astro Deployments as code. For example, if your team regularly creates and deletes Deployments for testing, you can use template files to avoid manually copying configurations in the Cloud UI.
...
In Astro CLI 1.9, you can use the `--dag-file` flag to run a specific DAG file without parsing all other DAGs in your directory. Specifying an individual DAG file makes it easier to troubleshoot errors for that DAG and results in faster execution of the command.

### Additional improvments

- When you run Airflow locally, you no longer need to enter credentials to log in to the Airflow UI.
- Local Airflow environments running with the Astro CLI now include access to the Airflow UI **Configurations** page (**Admin** > **Configurations**). This page shows the configuration for your environment, including environment variables and Astro Runtime defaults.
