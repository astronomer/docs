...
This document provides a summary of all changes made to the [Astro CLI](cli/quickstart.md). For general product release notes, go to [Astro Release Notes](release-notes.md).
...
We strongly recommend that all users install the Astro CLI and delete the `./astro` executable from local directories as soon as possible. For guidelines, read [Install the Astro CLI](cli/quickstart.md). As of February 2022, `./astro` will no longer be maintained by our team. With that said, the release of the Astro CLI does not have any impact on your existing Deployments or DAGs.
