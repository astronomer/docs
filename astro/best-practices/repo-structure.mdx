...
1. Host one Astro project in one Git repository for deployment to one Astro Deployment (one-to-one).
2. Separate your DAGs from other files and maintain multiple repositories for one Astro Deployment (many-to-one).
3. Host one Astro project in one repository for deployment to multiple Astro Deployments (one-to-many).
...
All the CI/CD strategies that Astro supports require Git plus either [GitHub](https://github.com/) or [GitLab](https://about.gitlab.com/). Implementation of version control is a longstanding software development best practice that enables:
...
- You have separate teams managing Deployments and maintaining the DAGs that run on them.
...
### Option 2: one repository to one or multiple Deployments
