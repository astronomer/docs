...
3. Complete the following fields:
    ...
    > *The earliest available Airflow version on Astro Runtime is 2.1.1, see here for a list of [LTS supported releases](astro/runtime-version-lifecycle-policy.md#astro-runtime-lifecycle-schedule). There are no known risks for upgrading directly from Airflow 2.0 to Airflow 2.1.1 during migration.
    ...
    > *Some of the following recommendations for CPU and memory are smaller than what you would have in the equivalent source environment. Although you may not need more CPU or memory than what's recommended here, some environments might parse DAGs slower than in your source Airflow environment to start. Use these recommendations as a starting point, then adjust your resource usage after tracking your performance on Astro.
...
6. Set an email to receive alerts from your Deployment. See [Add Or Delete A Deployment Alert Email](configure-deployment-resources.md#add-or-delete-a-deployment-alert-email).

</TabItem>
<TabItem value="cli" label="Option 2: Astro CLI & Deployments-as-Code">

1. On your local machine, create a file called `config.yaml` in a directory with the same name as the source Airflow
...
4. Run the following command to push your configuration up to Astro and create your Deployment:
