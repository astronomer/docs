...
If your current worker type doesn't have the right amount of resources for your workflows, see [Deployment settings](deployment-settings.md) to learn about configuring worker types on your Deployments.
...
Deploying to Astro with DAG-only deploys enabled can make deploys faster in cases where you've only modified your `dags` directory. To enable the DAG-only deploy feature, see [Deploy DAGs only](deploy-dags.md).