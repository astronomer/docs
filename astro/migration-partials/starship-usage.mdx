...
You might have set Airflow connections and variables in the following places on your source Airflow environment:

- The Airflow UI (stored in the Airflow metadata database).
- Environment variables with your cloud provider.
- A secrets backend.

If you set your Airflow variables and connections in the Airflow UI, you can migrate those to Astro with Starship. You can check which resources will be migrated by going to **Admin** > **Variables** and **Admin** > **Connections** in the Airflow UI for your source Airflow environment.
...
2. Click **Get Token**.
3. If required, sign in to `cloud.astronomer.io`.
4. Copy the access token that appears after logging in. 
5. In the Airflow UI, paste the access token into the **Authentication Token** field. 
6. Click **Sign in**.
7. In the **Target Deployment** menu, select the Deployment to which you want to migrate your Airflow variables and connections, then click **Select**.
...
12. In the table that appears, check the box for each environment variable that you want to migrate to Astro, then click **Migrate**. After the migration is complete, the status **Migrated ✅** appears.
  ...
  Some environment variables or Airflow Settings can't be migrated to Astro. See [Global environment variables](https://docs.astronomer.io/astro/platform-variables) for a list of variables that you can't override on Astro.
...
5. Replace the following placeholder values:
    ...
    - `<your-astro-token>`: To retrieve an Astronomer token, go to `cloud.astronomer.io/token` and log in using your Astro credentials. After you log in, copy the token that appears and paste it as the value. 

6. Click **Trigger**. After the DAG successfully runs, all connections, variables, and environment variables in your Airflow environment are migrated to Astronomer.
