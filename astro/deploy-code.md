...
After the deploy completes, the footer in the Airflow UI for your Deployment shows Docker image tag information. Depending on how your organization deploys to Astro, the Docker image tag is either a unique identifier generated by a Continuous Integration (CI) tool or a timestamp generated by the Astro CLI on `astro deploy`. The Docker image tag represents a unique deploy of your Astro project and the Docker image that is running on the Airflow webserver. Currently, you can’t confirm that your code deployed successfully and is running on all Deployment components. The image tag information displayed in the Airflow UI footer identifies only the Docker image that is running on your webserver container, and not on your schedulers, triggerers, or workers. To confirm a code push, contact [Astronomer support](https://support.astronomer.io). 
