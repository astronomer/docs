...
When the deploy completes, the **Docker Image** and **DAG Bundle Version** fields in the Cloud UI are updated with the Deployment code version. The **Docker Image** field displays a unique identifier generated by a Continuous Integration (CI) tool or a timestamp generated by the Astro CLI on `astro deploy`. The **DAG Bundle Version** field displays a unique timestamp generated by the Astro CLI when DAGs are updated. If you do not have DAG-only deploys enabled, the DAG bundle version field isn't populated.

To confirm a deploy was successful, verify that the running versions of your Docker image and DAGs are the latest versions.

1. In the Cloud UI, select a Workspace and then select a Deployment.
2. Review the information in the **Docker Image** field to determine the the Deployment code version. The unique identifier in this field is generated by a Continuous Integration (CI) tool or a timestamp generated by the Astro CLI on `astro deploy`.
