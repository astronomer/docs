...
Running `astro deploy` without using additional flags in the Astro CLI pushes your entire Astro project to Astro. This includes your DAG code, `Dockerfile`, and Python packages. There are some scenarios where you might want to deploy only the `dags` directory of your Astro project. To do this, you must enable the DAG-only deploy feature on Astro.
...
After you enable the feature, anyone with the required permissions can trigger a DAG-only deploy. 
