...
Depending on what your Astro project contains, you might need to wait a few minutes for the Docker image to build. After the deploy completes, the footer of the Airflow UI for your Deployment shows a new Docker image tag. Depending on how your organization deploys to Astro, the Docker image tag is either a unique identifier generated by a Continuous Integration (CI) tool or a timestamp generated by the Astro CLI on `astro deploy`.

Both represent a unique deploy of your Astro project and the Docker image that is running on the Airflow webserver. Comparing the Docker image tag from your deploy and the Docker image tag shown in the footer of the Airflow UI is currently the best way to confirm that your deploy to Astro was successful.
