...
- Upgraded `astronomer-providers-logging` to 1.3.0, which adds the ability to create custom tags when exporting logs to Datadog, so you can easily filter, aggregate, and compare data. See [Export task logs to Datadog](view-logs.md#export-task-logs-to-datadog) for setup instructions.
