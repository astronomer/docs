...
def create_experiment_func(experiment_name, artifact_bucket, **context):
...
def scale_features_func(experiment_id: str):
...
def mlflow_tutorial_dag():
    create_experiment = PythonOperator(
        ...
        python_callable=create_experiment_func,
        op_kwargs={
    ...
    scale_features = PythonOperator(
        task_id="scale_features",
        python_callable=scale_features_func,
        op_kwargs={"experiment_id": EXPERIMENT_ID},
    )
    ...
    (create_experiment >> scale_features >> create_registered_model)
