...
Starting with Astro Runtime 11, Astronomer maintains a slim Astro Runtime image. Slim Astro Runtime images only include the dependencies required for the basic funcionality of Astro. These packages are:
...
If you wanted the same base image using Python 3.11, but you didn't want a slim image, you would remove the `-slim-` from the image tag like in the following example:
