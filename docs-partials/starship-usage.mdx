...
Some environment variables or Airflow Settings, like global environment variable values, can't be migrated to Astro. See [Global environment variables](https://www.astronomer.io/docs/astro/platform-variables) for a list of variables that you can't migrate to Astro.
...
3. Create a Deployment API token for the Deployment. The token should minimally have permissions to update the Deployment and deploy code. Copy this token. See [Create and manage Deployment API tokens](https://www.astronomer.io/docs/astro/deployment-api-tokens) for additional setup steps.
